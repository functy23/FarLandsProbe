...
> `BlockPos` 用有符号 32 位 `int` 存坐标，所以 **±2,147,483,647 是物理上限**。扩展的坐标编码把渲染/生成上限推到 int 边缘——但越过之后，坐标在 `int` 里回绕，**地形无法生成**。这就是 int32 极限；要再往前需要任意精度坐标（整套游戏 fork，例如 MCBig 的思路），mod 层面做不到。
