...
> ⚠️ **C²M Engine rewrites the chunk storage/async loading system and is incompatible with the extended 28/8/28 section packing.** When C2ME is detected, the mod **automatically falls back to vanilla section encoding** (log: `C²M Engine detected: 28/8/28 section encoding auto-disabled`). In a C2ME profile you can still remove the border / movement clamps / teleport bounds, but **far-lands deep exploration (beyond ±33,554,432 blocks) will not generate** — the vanilla packing can only represent that range. Use a **non-C2ME environment** to see the full far-lands breakdown.

## About terrain beyond the 32-bit limit

> `BlockPos` stores coordinates in a signed 32-bit `int`, so **±2,147,483,647 is the physical ceiling**. Past that, coordinates wrap around: the mod's "wrap coordinates" toggle folds the player's position back by 2³², so generation/rendering continue — **but the terrain you see is still generated by the 32-bit generator, at the wrapped (negative) coordinates**. It is *not* a genuinely new terrain beyond 32 bits (that would require arbitrary-precision coordinates, i.e. a whole game fork); it is the world folded at the int edge, with the noise/lighting corruption you are here to observe.

## Rendering caveats

> ⚠️ Near and beyond the ±2,147,483,647 edge, rendering can glitch: **chunk flashing / flickering, sections popping in and out, light flicker, blocks briefly disappearing** (the render octree and section encoding are working right at the int limit). This is expected — the mod is deliberately exploring the breakdown, not polishing it.
...
| Far-lands stability → Wrap coordinates past the int limit | On | Fold the world at ±2³¹ so exploration continues |
| Far-lands stability → Fix chunk generation at the int edge | On | Wrap-aware worldgen distance/cache + skip features at the edge |
...
| Far-lands stability → Skip pathfinding past ±2×10⁹ | On | Avoid pathfinding-array overflow |
...
4. **Extended coordinate encoding & stability patches**
   ...
   - `Aquifer`: long-math guard near the int edge (prevents the 4 GB grid-size OOM)
   - `WorldGenRegion` / `StaticCache2D`: wrap-aware chessboard distance and cache re-mapping so edge chunks generate instead of "Requested chunk unavailable"
   - `getHeight` outside the generation cache returns a safe value; `applyBiomeDecoration` skips features for the last chunks before the edge
   - `PathNavigation`: pathfinding is skipped past ±2×10⁹ (avoids negative array size)
   - `LayerLightSectionStorage` / `EntitySectionStorage` / `MineshaftPieces` / `Octree` / structure-skip guards
